	...
	def grab_and_send(self):
		...
		while(self.cap.isOpened()):
			...
			if(ret):
				...
				filtered1 = cv2.applyColorMap(filtered, cv2.COLORMAP_JET)
				#disparity = cv2.convertScaleAbs(stereo.compute(left_img, right_img))
				cv2.namedWindow("Disparity", cv2.WINDOW_NORMAL)
				cv2.resizeWindow("Disparity", 800, 800)
				self.image_left_publisher.publish(self.bridge.cv2_to_imgmsg(left_img_alt, "passthrough"))
				self.image_right_publisher.publish(self.bridge.cv2_to_imgmsg(right_img_alt, "passthrough"))
