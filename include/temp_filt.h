...
namespace librealsense
{
    ...
    public:
        ...
        void    update_configuration(const Mat& f);
    ...
    protected:
        ...
        {   
            ...
            // for (int i = 0       ; i < _current_frm_size_pixels; i++)
            //     cout << i << ", " << ((uint16_t *)frame)[i] << endl;

            // pass one -- go through image and update all
            for (size_t i = 0; i < _current_frm_size_pixels - 1; i++)
            {   
                // cout << i << endl;
                // cout << &frame[i] << endl;
                T cur_val = frame[i];
    ...
    private:
        ...
        std::vector<uint16_t>    _last_frame;                // Hold the last frame received for the current profile
