...
namespace librealsense
{
    ...
    const float temp_alpha_default = 0.2f;
    ...
    const uint8_t temp_delta_default = 60;
    ...
    void temporal_filter::process_frame(Mat& frame)
    {   
        ...
        if (frame.isContinuous()) {
            temp_jw_smooth<uint16_t>((frame.ptr<uint16_t>(0)), _last_frame.data(), _history.data());
        }
        else
    ...
    void  temporal_filter::update_configuration(const Mat& f)
    {
